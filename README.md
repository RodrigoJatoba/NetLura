# Netflix Clone - Tema Dark/Light Mode


**(Imersão front end com IA - ALURA)** <img width="1895" height="947" alt="image" src="https://github.com/user-attachments/assets/9b6bd026-6e22-4c14-b8c0-43b931ea6aa9" />


Este projeto é um clone da página de seleção de perfis da Netflix com funcionalidade completa de alternância entre temas dark e light, focado em manipulação de DOM e CSS avançado.

---

## 🚀 Sobre o projeto

O **NetLura** foi desenvolvido durante a **Imersão Front-End da Alura**, com o objetivo de recriar a interface da Netflix utilizando HTML, CSS e JavaScript puro. 

O foco principal foi a implementação de um sistema de temas (Dark/Light) que utiliza conceitos de **Neumorfismo**, onde as sombras se adaptam para criar um efeito de relevo tátil, proporcionando uma experiência de usuário moderna e fluida.

---

## ✨ Funcionalidades

* 🌗 **Toggle de Tema:** Alternância entre Dark e Light mode com persistência no `localStorage`.
* 👥 **Seleção de Perfis:** Interface fiel à original para escolha de usuário.
* 🎨 **Design Neumórfico:** Botões e elementos visuais com profundidade baseada em sombras.
* 📱 **Responsividade:** Layout adaptável para diferentes tamanhos de tela.
* 🔍 **Filtros Dinâmicos:** Uso de `drop-shadow` em CSS para manter o contraste da logo em diferentes fundos.

---

## 🛠️ Tecnologias utilizadas

* **HTML5** - Estruturação semântica.
* **CSS3** - Variáveis, Flexbox e animações.
* **JavaScript** - Lógica de troca de temas e manipulação de estado.

---

## 📁 Estrutura do projeto

```text
.
├── assets/                 # Imagens e Avatares
├── catalogo-netflix/       # Página do catálogo
├── index.html              # Tela de seleção de perfis
├── index.js                # Lógica global
├── style.css               # Estilização e Variáveis
└── README.md               # Documentação
```
---

## 🧠 Aprendizados e Desafios

Durante o desenvolvimento deste projeto, foram consolidados conceitos fundamentais de desenvolvimento web:

* 🎨 **CSS Variables:** Essencial para gerenciar cores e sombras de forma centralizada, permitindo a troca de temas com apenas uma alteração de atributo.
* ⚙️ **DOM Manipulation:** Captura de eventos de clique e alteração dinâmica de classes para interatividade em tempo real.
* 💾 **Web Storage:** Implementação do `localStorage` para garantir que a preferência de tema do usuário seja lembrada em visitas futuras.
* 📂 **Case Sensitivity:** Ajuste rigoroso de caminhos de arquivos para garantir a compatibilidade com servidores Linux (GitHub Pages).

---

## 📈 Roadmap de Evolução

- [x] Implementação de Switch Neumórfico com suporte a temas.
- [x] Persistência de tema com LocalStorage.
- [x] Otimização de caminhos de assets para o deploy.
- [ ] **Integração TMDB API:** Alimentação dinâmica dos títulos do catálogo.
- [ ] **Micro-interações:** Animação de entrada "Netflix Intro" ao selecionar um perfil.

---

## 🤝 Contato

**Rodrigo Jatobá** 

[![github](https://img.shields.io/badge/GitHub-000?style=for-the-badge&logo=github&logoColor=white)](https://github.com/RodrigoJatoba)
[![linkedin](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/rodrigo-jatobá-9ab11b238/)
[![outlook](https://img.shields.io/badge/Microsoft_Outlook-0078D4?style=for-the-badge&logo=microsoft-outlook&logoColor=white)](mailto:rodrigosouza_al@hotmail.com)

---

**Desenvolvido por Rodrigo Jatobá** *durante a Imersão Front-End da Alura.*
